...
import { INITIAL_PLACES } from './sampleData';
...
import KakaoMapGuide from './components/KakaoMapGuide';
import UploadModal from './components/UploadModal';
import { Upload, List, X, Globe, MapPin, Sparkles as Sparkles2 } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const cachedPlaces = localStorage.getItem('worldcup_map_places');
    ...
    if (cachedPlaces) {
      ...
      localStorage.setItem('worldcup_map_places', JSON.stringify(INITIAL_PLACES));
  ...
  const handlePostReaction = (placeId, reactionType) => {
    ...
    setPlaces(updated);
    localStorage.setItem('worldcup_map_places', JSON.stringify(updated));
  };
  ...
  const handleApproveSubmission = (subId) => {
    ...
    localStorage.setItem('worldcup_map_places', JSON.stringify(nextPlaces));
  ...
  const handleBulkImport = (newPlaces) => {
    ...
    localStorage.setItem('worldcup_map_places', JSON.stringify(updatedPlaces));
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-red-650 selection:text-white pb-12 flex flex-col">
      ...
      <footer className="mt-16 border-t border-zinc-900/60 pt-6 text-center select-none text-zinc-500 text-[10.5px] font-mono leading-relaxed px-4 max-w-7xl mx-auto">
        <p>© 2026 월드컵맵 (WorldCup Map). All soccer cheering communities in Seoul united.</p>
        <p className="mt-1 text-zinc-650">Designed with modern visual dark neon tactical displays. No real database required for pre-launch evaluation.</p>
      </footer>
    </div>
  );
}
